...
- For tools, change the hand model path with `equipment_scene_path_template`.
  Use `%d` wherever the tier number should be inserted.
- For tools, change the socket offset with `equipment_attachment_profile_path`.
- For tools, change gathering/attack animation choices with
