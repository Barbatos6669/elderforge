"""Copy the edited T1 sickle mesh to other tiers while preserving tier colors.

Run with Blender from the project root:

    blender --background --python tools/blender/copy_sickle_t1_mesh_to_tiers.py

The script treats `assets/equipment/tools/sickles/t1/source/t1_sickle.blend`
as the shape master. It saves tier-specific `.blend` files for T2-T8, recolors
the tier-colored sickle blade material, and exports each runtime GLB.
"""
...
SOURCE_BLEND = SICKLE_ROOT / "t1" / "source" / "t1_sickle.blend"
...
def main() -> None:
    ...
    for tier in range(1, 9):
        ...
        root = _find_t1_root()
        if root is None:
            raise RuntimeError("Could not find root object 'T1Sickle' in the T1 sickle source.")

        root.name = f"T{tier}Sickle"
        root["elderforge_note"] = (
            f"One-handed T{tier} sickle copied from the edited T1 shape. "
            f"Edit this blend and re-export t{tier}_sickle.glb."
        )
        _rename_meshes(root, tier)
        _retier_materials(root, tier)

        blend_path = SICKLE_ROOT / f"t{tier}" / "source" / f"t{tier}_sickle.blend"
        ...
        print(f"Exported T{tier} sickle to {export_path}")


def _find_t1_root() -> bpy.types.Object | None:
    root = bpy.data.objects.get("T1Sickle")
    if root is not None:
        return root

    for obj in bpy.context.scene.objects:
        if obj.parent is None and "sickle" in obj.name.lower():
            return obj
    return None
