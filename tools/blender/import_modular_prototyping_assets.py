r"""Convert the Free 3D Modular Game Assets FBX files into Blend sources.
...
def asset_id_from_stem(stem: str) -> str:
    words = re.findall(r"[A-Z]?[a-z]+|[A-Z]+(?=[A-Z]|$)|\d+", stem.replace(" ", "_"))
...
def matching_assets(only: str) -> list[Path]:
    ...
    if not matches:
        raise RuntimeError(f"Could not find modular prototyping asset matching '{only}'.")
    return matches
