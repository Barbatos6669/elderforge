r"""Convert the Stylized Nature MegaKit FBX files into editable Blend sources.
...
The FBX files are kept as third-party raw input. The generated .blend files are
the artist-editable sources, and the generated glTF files are the runtime Godot
models.
"""
...
PACK_ROOT = PROJECT_ROOT / "assets" / "models" / "environment" / "stylized_nature_megakit"
...
def remap_images_to_shared_textures() -> None:
    for image in bpy.data.images:
        ...
        texture_path = TEXTURES_ROOT / image_name
...
def import_fbx_to_source(fbx_path: Path, asset_id: str) -> bpy.types.Object:
    ...
    root["elderforge_note"] = (
        "Stylized Nature MegaKit converted source. Edit this .blend, then run "
        "tools/blender/export_stylized_nature_megakit.py to refresh the runtime glTF."
    )
...
def matching_assets(only: str) -> list[Path]:
    ...
    if not matches:
        raise RuntimeError(f"Could not find nature asset matching '{only}'.")
    return matches
